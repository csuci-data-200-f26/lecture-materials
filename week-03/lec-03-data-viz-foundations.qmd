...
- Visualizations can convey a lot about data, but they are not accessible to everyone, such as people who have a visual impairment. 
...
Today we will cover data represented in visuals but next week we will touch on different data representations.
...
knitr::include_graphics("img/ggplot-illustration.png")
```
...
- When can we use a bar plot? **For a categorical variable**
- What does this bar plot convey? **Counts of each level**
...
- When can we use a histogram? **For a numerical variable**
- What does this histogram convey? **How the values are distributed, shape and spread**
...
```{r}
#| echo: true
#| fig-alt: "Same histogram as previous slide, except with much finer detail since each bin is very thin. This also results in a much smaller y axis, since there are fewer points in each interval."
ggplot(babies, aes(x = bwt)) +
...
```{r}
#| echo: true
#| fig-alt: "Same histogram as previous slide, except with slightly finer detail since each bin is slightly smaller. This also results in a slightly smaller y axis, since there are fewer points in each interval."
...
```{r}
#| echo: true
#| fig-alt: "Same histogram as previous slide, except with less detail since each bin is thicker. This also results in a much larger y axis, since there are more points in each interval."
...
```{r}
#| echo: true
#| fig-alt: "Same histogram as previous slide, except with much less detail since each bin is much thicker. This also results in a much larger y axis, since there are more points in each interval."
...
- How would the distribution change if Peter Dinklage (135 cm, 4' 5'') were to join our class?
...
Note that the y axis still shows as a count. We will learn how to change the axis labels later.
...
What type of variables can we display with a standardized bar plot? **Two categorical variables**
...
Let's look at what a single box plot is!
:::
...
- Here I overlayed the values, so you can see a boxplot provides summary statistics about how the data is distributed
...
Later on we will learn statistical modeling, during which we will numerically define the relationship between gestation and birth weight. 
...
#| fig-align: center
#| fig-cap: "Artwork by Allison Horst."
#| fig-alt: "Image showing three penguins with different features because they are different species: Chunstrap, Gentoo, and Adelie."
