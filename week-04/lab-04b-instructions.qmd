...
For the filtering code I provided, you will just need to change what is in the quotations (e.g., "AgeGroups").
You can add a code chunk using count to see what options are in the data.
For example, the code below will show what options are in the `event` variable.
```{r}
#| echo: true
count(hep_data, event)
```

Once you are done filtering the data, check how many rows are in your filtered data.
If there are zero rows, then something went wrong in the filtering.

When you have filtered your data successfully you are ready to make your plot.
I recommend first making a scatterplot of your variables. Then you can use `geom_line()` instead of `geom_point()`.
