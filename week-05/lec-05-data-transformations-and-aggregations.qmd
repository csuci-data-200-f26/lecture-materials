---
title: "Data 200: Lecture 5"
subtitle: "Data Transformations and Aggregations"
...
- Things will get messy and more difficult to read and debug as we deal with more complex code.
...
- We will end up with too many objects in `Environment`. There is a more efficient way to code.
...
:::{.column width="65%"}
1. Combine 4, 8, and 16, `and then`  
2. Take the mean   `and then`  
3. Round the output
:::
...
```{r}
lapd <- read_csv("https://csuci-data-200-f26.github.io/lecture-materials/data/Police_Payroll.csv") 
```
...
`clean_names()` from the `janitor` package makes column names follow tidyverse style!
```{r}
library(janitor)
...
1. Now the data is being saved into the `lapd` object, instead of just being printed.
...
`as.logical()` - makes a vector logical 
